...
parfor j = 1:repeatNum
    ...
    data_planar_t1(j) = prod(get_outage_probability(sp,cp,threshold,m,k,method));
    % t2
    threshold = TesThereshold(2);
    data_planar_t2(j) = prod(get_outage_probability(sp,cp,threshold,m,k,method));
    % t3
    threshold = TesThereshold(3);
    data_planar_t3(j) = prod(get_outage_probability(sp,cp,threshold,m,k,method));
    ...
    % t1
    threshold = TesThereshold(1);
    data_cuboidal_t1(j) = prod(get_outage_probability(sp,cp,threshold,m,k,method));
    % t2
    threshold = TesThereshold(2);
    data_cuboidal_t2(j) = prod(get_outage_probability(sp,cp,threshold,m,k,method));
    % t3
    threshold = TesThereshold(3);
    data_cuboidal_t3(j) = prod(get_outage_probability(sp,cp,threshold,m,k,method));
